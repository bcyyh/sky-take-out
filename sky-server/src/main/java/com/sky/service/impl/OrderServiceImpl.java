...
//        checkOutOfRange(addressBook.getCityName()+ addressBook.getDistrictName()+addressBook.getDetail());
    ...
    public OrderPaymentVO payment(OrdersPaymentDTO ordersPaymentDTO) throws Exception{
        ...
        //将订单号传入
        String orderNumber1 = ordersPaymentDTO.getOrderNumber();
        //调用微信支付接口，生成预支付交易单
...
//    把 Entity 里的数据拷贝并加工成 VO 返回给前端。
    ...
    public PageResult conditionSearch(OrdersPageQueryDTO ordersPageQueryDTO) {
        //进行分页查询 需要传入的是 页面大小 和 页数
        PageHelper.startPage(ordersPageQueryDTO.getPage(), ordersPageQueryDTO.getPageSize());
        // 调用mapper方法进行查询 page中封装的为Orders对象
        Page<Orders> page = orderMapper.pageQuery(ordersPageQueryDTO);
        // 部分订单状态，需要额外返回订单菜品信息，将Orders转化为OrderVO 商家才容易出餐更好看
    ...
    private List<OrderVO> getOrderVOList(Page<Orders> page) {
        //用Vo数据来封装给前端
        List<OrderVO> orderVOList = new ArrayList<>();  //创建一个集合
        List<Orders> ordersList = page.getResult();
        //“保鲜盒”里把那几个“苹果”拿出来，装进一个纯粹的“袋子”里 装的是你设置大小的袋子
        if (!CollectionUtils.isEmpty(ordersList)) {  //集合不为空
    ...
    private String getOrderDishesStr(Orders orders) {
        ...
        // 将该订单对应的所有菜品信息拼接在一起 加了分号所以直接这样就行
    ...
    public void cancel(OrdersCancelDTO ordersCancelDTO) {
        Orders ordersDB = orderMapper.getById(ordersCancelDTO.getId());  //用前端的数据获得数据中的旧数据
        //支付状态
        Integer payStatus = ordersDB.getPayStatus();
        /*if (payStatus == Orders.PAID) {
            String refund = weChatPayUtil.refund(
    ...
    public void delivery(Long id) {
        Orders ordersDB = orderMapper.getById(id);
        // 校验订单是否存在，并且状态为3 订单不存在 且 状态不是待派送状态 抛出异常
